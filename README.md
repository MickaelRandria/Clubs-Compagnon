...
1. **Les idées** — n'importe qui saisit un pseudo et propose un nom. Propositions illimitées, chacune affichée comme une carte.
2. **Le vote** — ouvert manuellement depuis **Réglages FC 27** (au moins une proposition). Les propositions sont alors figées.
   Chaque **pseudo exact** vote une seule fois ; un second vote du même pseudo est refusé.
...
- Les pseudos sont conservés tels que saisis ; aucune authentification ni normalisation.
- Les réglages sont publics, comme les autres actions : ne pas présenter le profil décoratif du bandeau comme une authentification.
- Une fiche joueur par pseudo exact, un poste secondaire facultatif. La modification exige de ressaisir le pseudo, sans vérifier l'identité.
