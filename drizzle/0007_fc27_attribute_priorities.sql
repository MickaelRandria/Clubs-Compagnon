-- Feuille à recopier : ordre de dépense des points d'attribut choisi par le joueur.
-- Tableau ordonné de clés AttributeKey (shared/data/archetypes.ts), 5 au maximum.
-- Vide pour les fiches créées avant cette migration : l'ordre par défaut est alors calculé côté client.
ALTER TABLE "fc27_player_profiles" ADD COLUMN "attribute_priorities" text[] DEFAULT '{}'::text[] NOT NULL;--> statement-breakpoint
ALTER TABLE "fc27_player_profiles" ADD CONSTRAINT "fc27_player_attribute_priorities_check" CHECK (cardinality("fc27_player_profiles"."attribute_priorities") <= 5);--> statement-breakpoint
-- fc27_snapshot utilise to_jsonb(p) : la nouvelle colonne remonte sans modification.
-- Seule la branche 'player' de fc27_dispatch change ; le reste est repris tel quel de 0006.
